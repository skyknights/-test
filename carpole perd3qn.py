...
TAU = 0.005                       
class SumTree(object):            # class SumTree(object): 是网上的   这里就是一个tree  存优先级的  模板是固定的
    """
...
class Memory(object):     # class Memory(object) : 是网上的     模板是固定的
...
class DQN():                                                            #仿照别人的自己写的DQN 	核心代码
  ...
  def create_Q_network(self):             
    ...
    self.soft_replace = [tf.assign(t, (1 - TAU) * t + TAU * e)                                  #  将原来的硬更新改写成自己的软更新方式   TAU设置为0.005
...
def main():                                                  #def main函数按照修订的方法自己写的
  ...
  for episode in range(EPISODE):
    ...
    if episode % 20 == 0:                                #每20个取一个点 
      ...
      number_pridddqn.append(episode)
      rewards_pridddqn.append(ave_reward)
      print (number_pridddqn)
      print (rewards_pridddqn,epsilon)
...
number_pridddqnpd = pd.DataFrame(number_pridddqn)        #生成csv  保存跑出来的数据
rewards_pridddqnpd =pd.DataFrame(rewards_pridddqn)

number_pridddqnpd.to_csv('datapridddqn1.csv')
rewards_pridddqnpd.to_csv('datapridddqn2.csv')
